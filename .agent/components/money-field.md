...
- **Currencies dropdown**: Inline part is placed on the right side of the field as inline dropdown element and has fixed width of 1rem (to display only currency symbol). It has similar styling as ComboBox dropdown but is completely borderless. The inline selector displays only the currency symbol, while the dropdown list displays both the symbol and the currency name. The dropdown part width should adjust to fit the content.
