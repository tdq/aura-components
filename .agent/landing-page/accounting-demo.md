...
The accounting demo extends the existing Aura Dashboard with a dedicated **Accounting** navigation section. It demonstrates `aura-components` in a realistic double-entry bookkeeping context, showcasing `MoneyField`, `DatePicker`, `GridBuilder` (with money columns), `ChartBuilder`, and `TabsBuilder` together in domain-appropriate screens.
