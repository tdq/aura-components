...
function createFooter(): HTMLElement {
    ...
    topBorder.style.cssText = 'background: linear-gradient(90deg, transparent 0%, rgba(103,80,164,0.4) 30%, rgba(125,82,96,0.4) 70%, transparent 100%);';
    ...
    brand.innerHTML = `
        <div class="flex items-center gap-px-12 mb-px-16 cursor-pointer" id="footer-logo">
            <div class="w-9 h-9 rounded-large flex items-center justify-center flex-shrink-0" style="background: linear-gradient(135deg, #6750A4, #7D5260);">
    ...
    socialCol.innerHTML = `
        ...
        <div class="flex flex-col gap-px-12">
            <a href="https://github.com" target="_blank" class="flex items-center gap-px-12 text-body-medium text-on-surface-variant hover:text-primary transition-colors duration-200 group" style="opacity: 0.7;">
            ...
            <a href="https://npmjs.com" target="_blank" class="flex items-center gap-px-12 text-body-medium text-on-surface-variant hover:text-primary transition-colors duration-200" style="opacity: 0.7;">
                <div class="w-8 h-8 rounded-medium flex items-center justify-center border" style="border-color: rgba(121,116,126,0.15); background: var(--md-sys-color-surface);">
    ...
    bottomBar.innerHTML = `
        ...
        <p class="text-label-medium text-on-surface-variant flex items-center gap-px-8" style="opacity: 0.5;">
            Built with
            <span style="background: linear-gradient(135deg, #6750A4, #7D5260); -webkit-background-clip: text; -webkit-text-fill-color: transparent; background-clip: text; font-weight: 600;">Aura Components</span>
            &amp; RxJS
        </p>
    `;
